...
项目背景（提示词默认）：本仓库是 **模型评测与选型工作台**，完整流程为：上传图片 → 选识别模型 → 识别衣物单品 → 上传/选预设模特 → 选试穿模型 → 生成模特+单品试穿图。非 C 端正式产品。详见 [README.md](./README.md)。
...
风格：lab-tool / SaaS
主题：浅色精致 / 深色高级 / …
项目背景：上传图片 → 选识别模型 → 识别单品 → 上传/选模特 → 选试穿模型 → 生成试穿图；对比模型效果以辅助选型
...
目标：体现完整流程（上传→识别→选模特→试穿）与选型，约 [N] 个 section
