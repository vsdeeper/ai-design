...
本项目是 **AI 识别虚拟试衣台**（`ai-recognition-virtual-fitting-bench`）：用于跑通并对比 **「AI 识别衣物 → AI 试穿」** 的业务流程测试台，而非面向 C 端的正式消费产品。

核心目标：

1. **识别** — 对比不同衣物识别模型的结果
2. **试穿** — 将识别结果接入不同虚拟试穿模型，对比生成效果
3. **决策** — 在同一流程下并排/对照实验，选出更优、更合适的模型组合

设计语气宜偏 **评测工作台 / 实验台**：冷静、可信、便于对照；避免泛泛「智慧零售营销站」话术。产品全称可写 `FitBench` 或仓库名。详见 [README.md](./README.md)。
...
项目背景：跑通「识别衣物 → AI 试穿」流程，对比不同模型生成效果以辅助选型
...
背景：AI 识别衣物 → AI 试穿，对比模型效果做选型。受众：[受众]，风格：[关键词]，浅色/深色：[选一]。
...
主题范式：浅色精致
语气：冷静、可信、对照实验感
项目背景：跑通「识别衣物 → AI 试穿」，并排对比不同模型效果以辅助选型

Section 数量与清单：
1. Hero — 工作台定位：识别 → 试穿 → 选型
2. Pipeline — 业务流程三段：识别 / 试穿 / 决策
3. Compare — 多模型并排对照（识别结果或试穿图）
...
目标：约 [N] 个 section，视觉质量优先；体现「识别 → 试穿 → 选型」
行业：AI 衣物识别 + 虚拟试穿（模型评测）
受众：[算法/产品/业务选型决策者]
风格关键词：[如 editorial / lab-tool / SaaS]
气质：冷静、可信、对照实验感（非 C 端营销）
主题：<[浅色精致 / 深色高级 / …] 选 1 个>
项目背景：对比不同识别与试穿模型的生成效果，帮助选出更合适的模型组合
...
背景：AI 识别衣物 → AI 试穿，模型效果对照与选型。约 [N] 个 section，风格：[关键词]。
...
气质：冷静、可信、对照实验感
主题：浅色精致
项目背景：跑通「识别衣物 → AI 试穿」，并排对比模型效果以辅助选型
...
产品：FitBench，品类：识别衣物 → 试穿的模型评测台，气质：[3 词]。
